...
#define DEBUG 1 // bandera para debuggear
...
void WebSocketClient::send_num(uint8_t* nums, uint16_t length) {
    DEBUG_WS("[WS] sending binary data");
    if (!client->connected()) {
        DEBUG_WS("[WS] not connected...");
        return;
    }

    // 1. send fin and type binary
    write(WS_FIN | WS_OPCODE_BINARY);

    // 2. send length
    if (length > 125) {
        write(WS_MASK | WS_SIZE16);
        write((uint8_t)(length >> 8));  // send the high byte
        write((uint8_t)(length & 0xFF));  // send the low byte
    } else {
        write(WS_MASK | (uint8_t)length);
    }
    ...
    for (int i = 0; i < length; ++i) {
        write(nums[i] ^ mask[i % 4]);  // Apply mask and send
    }
}
